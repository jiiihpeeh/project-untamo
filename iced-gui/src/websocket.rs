...
const WS_QUEUE_MAX: usize = 1000;

fn push_to_queue(result: Result<WsMessage, String>) {
    let mut queue = WS_MSG_QUEUE.lock().unwrap();
    if queue.len() >= WS_QUEUE_MAX {
    ...
    match connect_result {
        Ok(Ok((ws_stream, _))) => {
            ...
            if let Err(e) = write.send(Message::Text(ws_pair.to_string())).await {
                ...
                push_to_queue(Err(format!("Send error: {}", e)));
                        ...
                        ping = read.next() => {
                            match ping {
                                ...
                                Some(Ok(Message::Close(_))) => {
                                    ...
                                    push_to_queue(Err("Disconnected".to_string()));
        ...
        Err(_) => {
            eprintln!("WS: Connection timed out");
            WS_CONNECTED.store(false, Ordering::SeqCst);
            let _ = result_tx.send(Err("Connection timed out".to_string())).await;
        }
    }
}

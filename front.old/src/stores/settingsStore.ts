...
async function sendWebColors(colors: UserColors){
    ...
    let res = await axios.post(url, colorsClone, { headers: { token: token } })
    if(res.status !== 200){
...
const useSettings = create<UseSettings>()(
    persist(
      (set, get) => ({
            ...
            setColorMode: (mode) => set({ colorMode: mode, isLight: mode === ColorMode.Light }),
            ...
            loadColorScheme: (theme) => {
                if(!(theme in get().webColors)) return
